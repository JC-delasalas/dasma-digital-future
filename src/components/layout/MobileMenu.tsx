  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 lg:hidden animate-fade-in">
      <div className="absolute right-0 top-0 h-full w-4/5 sm:w-3/4 max-w-sm bg-white shadow-lg">
        ...
        <nav className="p-4">
          <ul className={`space-y-3 ${isSeniorMode ? 'text-xl' : 'text-base'}`}>
            <li>
              <Link to="/" className="flex items-center p-3 hover:bg-gov-neutrals-gray100 rounded-lg" onClick={onClose}>
            ...
            <li>
              <Link to="/transparency" className="flex items-center p-3 hover:bg-gov-neutrals-gray100 rounded-lg" onClick={onClose}>
          ...
          <div className="border-t border-gov-neutrals-gray200 mt-6 pt-4 space-y-4">
            <div className="flex items-center justify-between">
              <span className={`${isSeniorMode ? 'text-lg' : 'text-base'} font-medium`}>Accessibility</span>
            </div>
            <div className="flex items-center justify-between">
              <span className={`${isSeniorMode ? 'text-base' : 'text-sm'}`}>Senior Mode</span>
              <SeniorModeToggle isSeniorMode={isSeniorMode} toggleSeniorMode={() => {}} />
            </div>
            <div className="flex items-center justify-between">
              <span className={`${isSeniorMode ? 'text-base' : 'text-sm'}`}>Text to Speech</span>
              <TextToSpeech isSeniorMode={isSeniorMode} isSpeechActive={false} toggleSpeech={() => {}} />
            </div>
          </div>

          <div className="border-t border-gov-neutrals-gray200 mt-6 pt-4">
            <div className="flex flex-col space-y-2">
              <Button variant="outline" size={isSeniorMode ? "lg" : "default"} className="w-full justify-start" onClick={onClose}>
